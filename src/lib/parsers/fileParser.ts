...
export function parseJSON(content: string): ParseResult {
  try {
    ...
    if (typeof parsed === 'object' && parsed !== null) {
      const dataArray =
        parsed.data || parsed.certificates || parsed.records || parsed.items;
    ...
    return {
      success: false,
      error:
        'JSON must be an array of objects or contain a "data"/"certificates" array',
    };
  } catch (e) {
    return {
      success: false,
...
export function parseExcel(buffer: ArrayBuffer): ParseResult {
  try {
    ...
    if (jsonData.length === 0) {
      return {
        success: false,
        error: 'Excel sheet is empty or has no data rows',
      };
    }
    ...
    return {
      success: false,
      error: `Failed to parse Excel file: ${e instanceof Error ? e.message : 'Unknown error'}`,
    };
  }
}
...
export async function parseFile(file: File): Promise<ParseResult> {
  ...
  if (format === 'unknown') {
    return {
      success: false,
      error:
        'Unsupported file format. Please upload CSV, JSON, or Excel (.xlsx) files.',
    };
  }
