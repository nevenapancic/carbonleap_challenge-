...
import {
  uploadHbeCertificates,
  uploadSafCertificates,
  uploadFuelEuCertificates,
} from './actions';
import {
  generateCSVTemplate,
...
export default function UploadForm({ sources }: Props) {
  ...
  const getCertificateType = (): CertificateType | null => {
    ...
    if (
      sourceName.includes('fueleu') ||
      registryType.includes('fueleu') ||
      sourceName.includes('maritime') ||
      registryType.includes('maritime')
    ) {
      return 'fueleu';
    }
      ...
      <CardContent sx={{ p: 4 }}>
          ...
          <Box sx={{ mb: 3 }}>
            <Alert severity='warning' sx={{ mb: 2 }}>
              ...
              <Typography variant='body2'>
                {uploadState.validCount} certificates imported successfully, but{' '}
                {uploadState.invalidCount} rows had issues.
              </Typography>
            </Alert>
            <Button
              variant='contained'
            ...
            if (analysis?.type === 'wrong_format') {
              return (
                ...
                >
                  <Typography
                    variant='subtitle1'
                    sx={{ color: '#92400e', fontWeight: 600, mb: 1 }}
                  >
                    Wrong file format?
                  </Typography>
            ...
            if (analysis?.type === 'row_errors' && analysis.rows) {
              return (
                ...
                >
                  ...
                  >
                    Issues found in {analysis.rows.length} row
        ...
        <form onSubmit={handleSubmit}>
              ...
              >
                ...
                <Box sx={{ textAlign: 'left' }}>
                  ...
                  <Typography variant='body2' sx={{ color: 'grey.400' }}>
                    ...
                    {detectFileFormat(file.name).toUpperCase()}
                  </Typography>
                </Box>
                <IconButton
                  size='small'
          ...
          <Button
            ...
            fullWidth
            disabled={
              !file || !selectedSource || uploadState.status === 'uploading'
            }
            sx={{ mt: 2 }}
          >
