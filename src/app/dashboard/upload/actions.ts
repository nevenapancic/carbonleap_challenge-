...
import {
  validateFuelEuRows,
  type FuelEuMaritimeCertificate,
} from '@/lib/validation/fueleu';
...
async function parseFileContent(file: File): Promise<ParseResult> {
  ...
  if (format === 'unknown') {
    return {
      success: false,
      error:
        'Unsupported file format. Please upload CSV, JSON, or Excel files.',
    };
  }
...
export async function uploadHbeCertificates(
  formData: FormData,
  ...
  const normalizedRows = parseResult.data.map((row) => {
    ...
    if ('double_counting' in normalized) {
      normalized.double_counting = String(
        transformBooleanField(normalized.double_counting),
      );
    }
  ...
  const validCertificates = validation.valid
    .map((v) => {
...
export async function uploadSafCertificates(
  formData: FormData,
): Promise<UploadResult> {
  const supabase = await createClient();
  ...
  const normalizedRows = parseResult.data.map((row) => {
    ...
    if ('corsia_eligible' in normalized) {
      normalized.corsia_eligible = String(
        transformBooleanField(normalized.corsia_eligible),
      );
    }
    if ('eu_red_compliant' in normalized) {
      normalized.eu_red_compliant = String(
        transformBooleanField(normalized.eu_red_compliant),
      );
    }
    return transformEmptyToNull(normalized);
  });
    ...
    .map((v) => {
      ...
      return {
        ...
        expiration_date: data.expiration_date
          ? parseDateToISO(data.expiration_date)
          : null,
        delivery_date: parseDateToISO(data.delivery_date),
        ...
        retirement_date: data.retirement_date
          ? parseDateToISO(data.retirement_date)
          : null,
...
export async function uploadFuelEuCertificates(
  formData: FormData,
): Promise<UploadResult> {
  const supabase = await createClient();
  ...
  const normalizedRows = parseResult.data.map((row) => {
    ...
    if ('rfnbo_subtarget_met' in normalized) {
      normalized.rfnbo_subtarget_met = String(
        transformBooleanField(normalized.rfnbo_subtarget_met),
      );
    }
    if ('ops_connected' in normalized) {
      normalized.ops_connected = String(
        transformBooleanField(normalized.ops_connected),
      );
    }
    if ('ops_exception_applied' in normalized) {
      normalized.ops_exception_applied = String(
        transformBooleanField(normalized.ops_exception_applied),
      );
    }
    if ('document_of_compliance_issued' in normalized) {
      normalized.document_of_compliance_issued = String(
        transformBooleanField(normalized.document_of_compliance_issued),
      );
    }
    return transformEmptyToNull(normalized);
  });
  ...
  const validFuelEuCerts = validation.valid
    .map((v) => {
      const data = v.result.success
        ? (v.result.data as FuelEuMaritimeCertificate)
        : null;
      if (!data) return null;
      return {
        upload_id: upload.id,
