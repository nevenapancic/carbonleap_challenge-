...
import {
  ...
  getPaginatedFuelEuCertificates,
} from './actions';
...
export default async function DashboardPage() {
  ...
  const safSource = sources.find(
    (s) => s.name === 'SAF' || s.name.toLowerCase().includes('saf'),
  );
  const fuelEuSource = sources.find(
    (s) =>
      s.name === 'FuelEU Maritime' || s.name.toLowerCase().includes('fueleu'),
  );
  const sourcesWithStats = await getCompanySourcesWithStats(company.id);

  let hbeStats = null;
  let initialCertificates: Awaited<
    ReturnType<typeof getPaginatedCertificates>
  > | null = null;

  if (hbeSource) {
    hbeStats = await getHbeStats(hbeSource.id, company.id);
    initialCertificates = await getPaginatedCertificates(
      hbeSource.id,
      company.id,
      1,
      5,
  ...
  let safStats = null;
  let initialSafCertificates: Awaited<
    ReturnType<typeof getPaginatedSafCertificates>
  > | null = null;

  if (safSource) {
    safStats = await getSafStats(safSource.id, company.id);
    initialSafCertificates = await getPaginatedSafCertificates(
      safSource.id,
      company.id,
      1,
      5,
  ...
  let initialFuelEuCertificates: Awaited<
    ReturnType<typeof getPaginatedFuelEuCertificates>
  > | null = null;

  if (fuelEuSource) {
    fuelEuStats = await getFuelEuStats(fuelEuSource.id, company.id);
    initialFuelEuCertificates = await getPaginatedFuelEuCertificates(
      fuelEuSource.id,
      company.id,
      1,
      5,
      ...
      >
        <Container maxWidth='xl'>
          <Box sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
            ...
            <Avatar
              sx={{ bgcolor: '#4ade80', color: 'black', fontWeight: 600 }}
            >
              {userInitials}
            </Avatar>
          </Box>
        ...
        {hbeSource && hbeStats && hbeStats.totalCertificates > 0 && (
          <Card sx={{ bgcolor: 'background.paper', mb: 4 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {initialCertificates &&
                initialCertificates.certificates.length > 0 &&
                hbeSource && (
                  <Box sx={{ mt: 2 }}>
        ...
        {safSource && safStats && safStats.totalCertificates > 0 && (
          <Card sx={{ bgcolor: 'background.paper', mb: 4 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {initialSafCertificates &&
                initialSafCertificates.certificates.length > 0 &&
                safSource && (
                  <Box sx={{ mt: 2 }}>
        ...
        {fuelEuSource && fuelEuStats && fuelEuStats.totalCertificates > 0 && (
          <Card sx={{ bgcolor: 'background.paper', mb: 4 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {initialFuelEuCertificates &&
                initialFuelEuCertificates.certificates.length > 0 &&
                fuelEuSource && (
                  <Box sx={{ mt: 2 }}>
                    <FuelEuCertificatesTable
                      ...
                      companyId={company.id}
                      initialCertificates={
                        initialFuelEuCertificates.certificates
                      }
          ...
          (!fuelEuStats || fuelEuStats.totalCertificates === 0) && (
            <Card sx={{ bgcolor: 'background.paper' }}>
              <CardContent sx={{ textAlign: 'center', py: 6 }}>
                ...
                <Link href='/dashboard/upload'>
                  <Button variant='contained'>Upload Certificates</Button>
                </Link>
              </CardContent>
            </Card>
          )}
